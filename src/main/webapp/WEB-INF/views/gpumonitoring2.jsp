                            ...
                            <div class="row">
                                ...
                                <div class="col-2" style="margin-bottom: auto;margin-top: auto">
                                    <table class="table text-center table-hover border-bottom">
                                        <tr>
                                            <td class="font-weight-bold" scope="col">MAX</td>
                                            <td scope="col" id="gpuclockmax"></td>
                                        </tr>
                                        <tr>
                                            <td class="font-weight-bold">min</td>
                                            <td id="gpuclockmin"></td>
                                        </tr>
                                    </table>
                                </div>
                            </div>
...
$.ajax({
    ...
    success:function(data){
        dataset = data.data.result[1].values;
        ...
        $('#gputempmax').text(Math.round(max)+" °C");
        $('#gputempmin').text(Math.round(min)+" °C");
    },
    error: function(err){
        console.log(err);
    }
...
$.ajax({
    ...
    success:function(data){
        ...
        for(var i = 0 ; i < dataset.length ; i++){
            data.data.result[1].values[i][1] = data.data.result[1].values[i][1]/Math.pow(10,9).toFixed(1);
            arr.push(data.data.result[1].values[i][1]);
        }

        dataset2 = data.data.result[1].values;

        max2 = Math.max.apply(null, arr);
        min2 = Math.min.apply(null, arr);

        $('#gpumemmax').text(max2.toFixed(2)+" %");
        $('#gpumemmin').text(min2.toFixed(2)+" %");
    },
    error: function(err){
        console.log(err);
    }
...
function tick2() {
    ...
    $.ajax({
        ...
        data:{},
        success:function(data){
            pushdata = [data.data.result[1].value[0],data.data.result[1].value[1]/Math.pow(10,9).toFixed(1)];
        },
        error: function(err){
            console.log(err);
        }
...
$.ajax({
    ...
    success:function(data){
        ...
        for(var i = 0 ; i < dataset.length ; i++){
            data.data.result[1].values[i][1] = data.data.result[1].values[i][1];
            arr.push(data.data.result[1].values[i][1]);
        }

        dataset3 = data.data.result[1].values;

        max3 = Math.max.apply(null, arr);
        min3 = Math.min.apply(null, arr);

        $('#gpuclockmax').text(max3+" Ghz");
        $('#gpuclockmin').text(min3+" Ghz");
    },
    error: function(err){
        console.log(err);
    }
...
var yScale3 = d3.scaleLinear()
    .domain([Number(Math.round(min3))-1, Number(Math.round(max3))+1]) // input
...
function tick3() {
    ...
    $.ajax({
        ...
        data:{},
        success:function(data){
            pushdata = [data.data.result[1].value[0],data.data.result[1].value[1]];
        },
        error: function(err){
            console.log(err);
        }
    ...
    yScale3.domain([Number(min3)-1, Number(max3)+1]);
