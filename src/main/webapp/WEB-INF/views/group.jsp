...
<body class="subpage">
	...
	function creategroup(){
		...
		if($("#group_name").val()==null||$("#group_name").val()==""){
			swal.fire("error","그룹명을 입력해주세요.","error");
			return false;
		}
		...
		if(type=="Save"){
			...
			if(groupNameCheck(group_name)!="false"){
				$.ajax({
		            url:"<%=cp%>/creategroup",
						...
						success : function(data) {
							swal.fire("success","등록되었습니다.","success");
							setTimeout(function() {
								location.reload();
							}, 2000);
						},
				...
				swal.fire("error","이미 등록된 그룹명입니다.","error");
				$("#group_name").val("");
				return false;
			}
	            ...
	            $.ajax({
			          url:"<%=cp%>/updateGroup",
							...
							success : function(data) {
								...
								swal.fire("success","수정되었습니다.","success");
								setTimeout(function() {
									location.reload();
								}, 2000);	
            ...
            $.ajax({
                url:"<%=cp%>/delGroup",
     				...
     				success : function(data) {
     					...
     					swal.fire("success","삭제되었습니다.","success");
     					setTimeout(function() {
     						location.reload();
     					}, 2000);
